...
docker build -f Dockerfile --tag=registry.okina.fr/mobiiti/flexible-transport:${PACKAGE_VERSION} --no-cache  --force-rm=true . --build-arg SSH_KEY="$(cat ~/.ssh/id_rsa)"
docker push registry.okina.fr/mobiiti/flexible-transport:${PACKAGE_VERSION}
