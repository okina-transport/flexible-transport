...
docker build --no-cache --tag=registry.okina.fr/mosaic/enki:${PACKAGE_VERSION} --force-rm=true . --build-arg SSH_KEY="$(cat ~/.ssh/id_rsa)"
docker push registry.okina.fr/mosaic/enki:${PACKAGE_VERSION}
